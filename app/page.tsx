...
export const revalidate = 60; // re-fetch from Supabase every 60s[cite: 4]
...
export default async function Home() {
  ...
  const featured = (projects ?? []).find((p) => p.featured);[cite: 4]
  ...
  return (
    <>
      ...
      <div className="relative z-10 bg-white dark:bg-neutral-950 w-full min-h-screen">[cite: 4]
        {/* Feathered edge that slides up over the gradient to soften the transition */}
        <div className="absolute left-0 right-0 h-40 -top-40 bg-gradient-to-b from-transparent to-white dark:to-neutral-950 pointer-events-none" />[cite: 4]
        
        <main className="max-w-5xl mx-auto px-6 py-24">[cite: 4]
          
          {/* HERO — asymmetric, type-led */}
          <section className="mb-20 max-w-3xl">[cite: 4]
            <TextReveal delay={0}>[cite: 4]
              <span className="inline-block text-xs font-medium tracking-wide uppercase text-purple-600 dark:text-purple-400 mb-4">
                AI-assisted developer
              </span>[cite: 4]
            </TextReveal>

            <TextReveal delay={0.15}>[cite: 4]
              <h1 className="text-4xl sm:text-5xl font-semibold leading-[1.1] tracking-tight text-neutral-900 dark:text-neutral-50 mb-6">[cite: 4]
                {featured ? (
                  featured.title
                ) : (
                  <>
            ...
            <TextReveal delay={0.3}>[cite: 4]
              <p className="text-lg text-neutral-600 dark:text-neutral-400 max-w-xl mb-8">[cite: 4]
                {featured?.description ??
                  "Web, ads, and everything between — built to look intentional, not templated."}[cite: 4]
              </p>
            </TextReveal>

            <TextReveal delay={0.45}>[cite: 4]
              <div className="flex gap-3">[cite: 4]
                <SpringButton
                  href="#work"
          ...
          <div id="work" className="mb-24">[cite: 4]
            <ProjectGrid categories={categories ?? []} projects={projects ?? []} />[cite: 4]
          </div>

          {/* ABOUT — story-led */}
          <FadeInSection>[cite: 4]
            <section id="about" className="mb-24 max-w-2xl scroll-mt-20">[cite: 4]
              <span className="inline-block text-xs font-medium tracking-wide uppercase text-purple-600 dark:text-purple-400 mb-4">
                About
              </span>[cite: 4]
              ...
              <div className="space-y-4 text-neutral-600 dark:text-neutral-400 leading-relaxed">[cite: 4]
                <p>
          ...
          {/* CONTACT */}
          <FadeInSection>[cite: 4]
            <section id="contact" className="mb-10 max-w-2xl scroll-mt-20">[cite: 4]
              <span className="inline-block text-xs font-medium tracking-wide uppercase text-purple-600 dark:text-purple-400 mb-4">
                Contact
              </span>[cite: 4]
              <h2 className="text-2xl sm:text-3xl font-semibold tracking-tight text-neutral-900 dark:text-neutral-50 mb-6">
              ...
              <div className="flex flex-wrap gap-3">[cite: 4]
                <SpringButton
